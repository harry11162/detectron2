...
class CondConvBottleneckBlock(CNNBlockBase):
    ...
    def forward(self, x, meta):
        routing_weight = self.route_func(meta)
...
class CondConvBasicStem(CNNBlockBase):
    """
    The standard ResNet stem (layers before the first residual block).
    """
...
class ResNet(Backbone):
    ...
    def forward(self, x, meta):
        ...
        outputs = {}
        routing_weight = self.route_func(meta)
        if isinstance(self.stem, CondConvBasicStem):
            x = self.stem(x, routing_weight)
        else:
            x = self.stem(x)
        ...
        if self.num_classes is not None:
            ...
            if "linear" in self._out_features:
                outputs["linear"] = x
        outputs["routing_weights"] = routing_weight
        return outputs
...
def build_meta_global_cond_conv_resnet_backbone(cfg, input_shape):
    """
    ...
    if cfg.MODEL.RESNETS.COND_CONV_ON_STEM:
        stem = CondConvBasicStem(
    ...
    return ResNet(stem, stages, out_features=out_features,
                  cond_conv_num_experts=cond_conv_num_experts,
                  cond_conv_dropout_rate=cond_conv_dropout_rate,
                  meta_channels=meta_channels,
                  routing_inner_channels=routing_inner_channels).freeze(freeze_at)
