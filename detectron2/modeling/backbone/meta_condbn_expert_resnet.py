...
class MLP(nn.Module):
    def __init__(self, num_layers, in_channels, inner_channels, out_channels):
...
class CondBNBasicBlock(CNNBlockBase):
    """
    The basic residual block for ResNet-18 and ResNet-34 defined in :paper:`ResNet`,
    with two 3x3 conv layers and a projection shortcut if needed.
    """

    def __init__(self, in_channels, out_channels, *, stride=1, norm="BN", num_experts=None):
        """
        Args:
            in_channels (int): Number of input channels.
            out_channels (int): Number of output channels.
...
class ResNet(Backbone):
    ...
    def __init__(self, stem, stages, num_classes=None, out_features=None, meta_channels=None, num_experts=None):
        """
        Args:
        ...
        self.routing_func = MLP(3, meta_channels, 64, num_experts)
        ...
        for name, stage in zip(self.stage_names, self.stages):
            for block in stage:
                if isinstance(block, CondBNBasicBlock):
                    x, gamma, beta = block(x, routing_weight)
                    gammas.append(gamma)
                    betas.append(beta)
                else:
                    x = block(x)
...
def build_meta_condbn_expert_resnet_backbone(cfg, input_shape):
    """
    ...
    num_experts = cfg.MODEL.RESNETS.COND_CONV_NUM_EXPERTS
    # fmt: on
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if depth in [18, 34]:
            if cond_conv_on_per_stage[idx]:
                stage_kargs["block_class"] = CondBNBasicBlock
                stage_kargs["num_experts"] = num_experts
            else:
    ...
    return ResNet(stem, stages, out_features=out_features, meta_channels=meta_channels, num_experts=num_experts).freeze(freeze_at)
