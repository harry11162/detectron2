...
class MyNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        """
        super().__init__()
        backbone = build_combine_meta_cond_conv_resnet_backbone(cfg, ShapeSpec(channels=3))
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        wt = self.ws[None, :, None].to(images.tensor.device) * metas[:, None, :]  # (N, D, 4)
        wt = wt.reshape(N, -1)  # (N, D*4)
        sinwt = torch.sin(wt)
        coswt = torch.cos(wt)
